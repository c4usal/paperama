"use client";

import Image from "next/image";
import { Bookmark, Compass, Hash, Search, Sparkles } from "lucide-react";

import { Button } from "@/components/ui/button";
import { useFeed } from "@/contexts/feed-context";
import type { FeedNavTab } from "@/types/feed";
import { cn } from "@/lib/utils";

const PRIMARY_NAV: { id: FeedNavTab; label: string; icon: typeof Sparkles }[] = [
  { id: "for-you", label: "For You", icon: Sparkles },
  { id: "saved", label: "Saved", icon: Bookmark },
  { id: "topics", label: "Topics", icon: Hash },
];

const SOCIAL_LINKS = [
  {
    label: "LinkedIn",
    href: "https://www.linkedin.com/in/ulofe-uduokhai-130a96373/",
    icon: LinkedInIcon,
  },
  {
    label: "X",
    href: "https://x.com/ulofish",
    icon: XIcon,
  },
  {
    label: "GitHub",
    href: "https://github.com/c4usal/paperama",
    icon: GitHubIcon,
  },
] as const;

function LinkedInIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className={className} fill="currentColor">
      <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 1 1 0-4.124 2.062 2.062 0 0 1 0 4.124zM7.119 20.452H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
    </svg>
  );
}

function GitHubIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className={className} fill="currentColor">
      <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
    </svg>
  );
}

function XIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className={className} fill="currentColor">
      <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
    </svg>
  );
}

function SocialLinks({ compact = false }: { compact?: boolean }) {
  if (compact) {
    return (
      <div className="mt-3 flex flex-col items-center gap-1.5">
        {SOCIAL_LINKS.map((link) => (
          <a
            key={link.label}
            href={link.href}
            target="_blank"
            rel="noopener noreferrer"
            title={link.label}
            aria-label={link.label}
            className="flex size-9 items-center justify-center rounded-xl text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
          >
            <link.icon className="size-4" />
          </a>
        ))}
      </div>
    );
  }

  return (
    <div className="mt-3 space-y-1 border-t border-border pt-4">
      {SOCIAL_LINKS.map((link) => (
        <a
          key={link.label}
          href={link.href}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-3 rounded-xl px-3 py-2 text-sm text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
        >
          <link.icon className="size-4 shrink-0" />
          <span>{link.label}</span>
        </a>
      ))}
    </div>
  );
}

export function FeedNavSidebar() {
  const { activeTab, setActiveTab, openSearch } = useFeed();

  return (
    <aside
      className={cn(
        "hidden h-full shrink-0 flex-col border-r border-border bg-card lg:flex",
        "w-[var(--feed-nav-width)]",
        "xl:w-[var(--feed-nav-width-expanded)]",
      )}
    >
      {/* Icon rail: lg only */}
      <div className="flex w-full flex-1 flex-col items-center px-2 py-6 xl:hidden">
        <Image
          src="/paperama-logo.jpg"
          alt="Paperama"
          width={40}
          height={40}
          className="mb-8 size-10 rounded-xl object-cover"
          priority
        />

        <Button
          type="button"
          variant="ghost"
          size="icon"
          className="mb-6 size-10 rounded-xl"
          onClick={openSearch}
          aria-label="Search"
          title="Search"
        >
          <Search className="size-5" />
        </Button>

        <nav className="flex w-full flex-col items-center gap-2">
          {PRIMARY_NAV.map((item) => (
            <div key={item.id} className="flex w-full flex-col items-center">
              <button
                type="button"
                title={item.label}
                onClick={() => setActiveTab(item.id)}
                className={cn(
                  "flex size-11 items-center justify-center rounded-xl transition-colors",
                  activeTab === item.id
                    ? "bg-rg-accent/10 text-rg-accent"
                    : "text-muted-foreground hover:bg-muted hover:text-foreground",
                )}
              >
                <item.icon className="size-5" />
                <span className="sr-only">{item.label}</span>
              </button>
              {item.id === "topics" ? <SocialLinks compact /> : null}
            </div>
          ))}
        </nav>
      </div>

      {/* Expanded sidebar: xl+ */}
      <div className="hidden min-h-0 flex-1 flex-col px-5 py-8 xl:flex">
        <div className="mb-10 flex items-center gap-3">
          <Image
            src="/paperama-logo.jpg"
            alt="Paperama"
            width={40}
            height={40}
            className="size-10 rounded-xl object-cover"
            priority
          />
          <div>
            <span className="text-xl font-semibold tracking-tight text-foreground">Paperama</span>
            <p className="mt-0.5 text-xs text-muted-foreground">TikTok, but it&apos;s peer-reviewed.</p>
          </div>
        </div>

        <label className="relative mb-10 block">
          <Search className="pointer-events-none absolute top-1/2 left-3.5 size-4 -translate-y-1/2 text-muted-foreground" />
          <input
            type="search"
            readOnly
            onFocus={openSearch}
            onClick={openSearch}
            placeholder="Search papers…"
            className="h-10 w-full cursor-pointer rounded-xl border border-border bg-muted/30 pr-3 pl-10 text-sm outline-none placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-3 focus-visible:ring-ring/50"
          />
        </label>

        <nav className="space-y-1.5">
          <p className="mb-3 px-3 text-[11px] font-medium tracking-wide text-muted-foreground uppercase">
            Menu
          </p>
          {PRIMARY_NAV.map((item) => (
            <div key={item.id}>
              <button
                type="button"
                onClick={() => setActiveTab(item.id)}
                className={cn(
                  "flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors",
                  activeTab === item.id
                    ? "bg-rg-accent/10 text-rg-accent"
                    : "text-foreground hover:bg-muted",
                )}
              >
                <item.icon className="size-[1.125rem] shrink-0" />
                <span>{item.label}</span>
              </button>
              {item.id === "topics" ? <SocialLinks /> : null}
            </div>
          ))}
        </nav>

        <div className="mt-auto rounded-xl bg-rg-accent/8 p-4">
          <div className="flex items-start gap-3">
            <Compass className="mt-0.5 size-4 shrink-0 text-rg-accent" />
            <p className="text-xs leading-relaxed text-muted-foreground">
              Tune your feed by saving papers — Paperama learns what you want next.
            </p>
          </div>
        </div>
      </div>
    </aside>
  );
}
